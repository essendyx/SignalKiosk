...
const playbackUrl = `${window.location.protocol}//${window.location.hostname}:${import.meta.env.VITE_PLAYBACK_PORT || "8090"}/playback`
...
const guidance = (): string => {
  if (locale.value === "de") {
    ...
    if (status.value.mode === "fallback") return "Kein aktives Preset verfuegbar. Setzen Sie ein Standard-Preset oder aktivieren Sie einen Zeitplan."
    if (status.value.mode === "default") return "Standardbetrieb aktiv. Zeitplaene sind optional und koennen den Betrieb zeitweise ueberschreiben."
    if (status.value.mode === "schedule") return "Ein Zeitplan steuert aktuell die Wiedergabe."
    return "Webhook-Override aktiv. Nach Ablauf wird automatisch zur regulaeren Logik zurueckgekehrt."
  }
...
const checklist = (): Array<{ label: string; ok: boolean; detail: string }> => {
  ...
  return [
    ...
    { label: "Zeitplan (optional)", ok: true, detail: hasSchedule ? "Aktiv" : "Kein aktiver Zeitplan - Standard-Preset laeuft dauerhaft" }
  ]
}
...
<template>
  <section class="page">
    ...
    <p class="page-subtitle">{{ locale === 'de' ? 'Uebersicht ueber den aktuellen Wiedergabestatus des Kiosks.' : 'Overview of the current kiosk playback status.' }}</p>
    <div class="toolbar">
      <a :href="playbackUrl" target="_blank" rel="noopener"><button>{{ locale === 'de' ? 'Playback im neuen Tab' : 'Open playback in new tab' }}</button></a>
    </div>
    <div class="status-grid">
      <div class="card" v-if="status">
        ...
        <p><strong>{{ locale === 'de' ? 'Gueltig bis' : 'Valid Until' }}:</strong> {{ status.ends_at || "-" }}</p>
      </div>
        ...
        <table class="data-table compact-table">
          ...
          <tbody>
            <tr v-for="item in checklist()" :key="item.label">
              <td>{{ item.label }}</td>
              <td><span :class="item.ok ? 'state-ok' : 'state-warn'">{{ item.ok ? (locale === 'de' ? 'Gruen' : 'Green') : (locale === 'de' ? 'Gelb' : 'Yellow') }}</span></td>
              <td>{{ item.detail }}</td>
            </tr>
          </tbody>
        </table>
